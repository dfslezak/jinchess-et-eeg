...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# Usar la version en 32 bits de Java (por alguna razon la version de 64 bits no esta andando bien)
# haciendo:
# 
#   cd C:\Program Files (x86)\Jin
#   C:\Program Files (x86)\Java\jre6\bin\java -jar jin.jar | C:\Python24\python -u hook.py
#
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
def gameCancelled ():
  # declarar los globales que vayamos a usar
  global state, board_n, board_o,mouse_filename,mousefile, code_gamecancelled,destPath

  # aca habra que hacer alguna magia
  print ">>>>>>>>>>>>>>> GAME CANCELLED <<<<<<<<<<<<<<<"

  # terminamos con un juego
  state = board_n = board_o = None
...
def gameEnded ():
  # declarar los globales que vayamos a usar
  global download_line, user, state, board_n, board_o, edf_filename, code_gameended,destPath
...
def pieceMoved ():
  # declarar los globales que vayamos a usar
  global board_n, line, board_ini, board_end, board_o, state, code_white, white_turn, code_black

  # nos buscamos el tablero nuevo
  board_n = line [board_ini:board_end]
  
  # Esto no va mas... estoy detectando el final con los mensajes de "forfeit", "cancelled", etc...
  # si la posicion no cambio...
  #if (board_o == board_n):
    # ...el partido termino...
    #gameEnded () 
    # ...y nosotros tambien
  ...
  # si la posicion cambio y estamos en pleno movimiento...
  #elif state == 1:
  if state == 1:
    # aca habra que hacer alguna otra magia
    print ">>>>>>>>>>>>>>> PIECE MOVED <<<<<<<<<<<<<<<"
    print line
    # ...mandar el codigo por el LPT
    # NODEBUG
  ...
  state = 1
  # de una forma u otra actualizamos el tablero viejo
  board_o = board_n
